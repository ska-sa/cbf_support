...
if __name__ == '__main__':
    ...
    parser.add_option('-a', '--maxswitches', type=int, default=64,
                      help='Number of switches in the system.')
    parser.add_option('-n', '--numsw', type=int, default=64,
    ...
    numsw = opts.numsw
    mswch = opts.maxswitches
    if strsw+numsw > mswch+1:
        rng = mswch+1
    else:
        rng = strsw+numsw

    for i in range(strsw,rng):
        hosts.append('10.96.{}.40'.format(i-1))
    ...
    def get_rates(switch_dict, ssh_list):
        ...
        # Get switch rates:
        speed_exp_bits  = ['b/s','Kb/s','Mb/s','Gb/s']
        speed_exp_bytes = ['B/s','KB/s','MB/s','GB/s']
        cmd = 'show interface ethernet rates'
        good_output = False
        all_output = run_threaded_cmd(ssh_list,cmd)
        try:
            for output in all_output:
                sw_name_idx = [i for i,s in enumerate(output) if 'dt-sw' in s][0]
                sw_name = output[sw_name_idx].split(' ')[0].split('-')[0]
                rates = [filter(None,output[y].split(' ')) for y in [i for i,s in enumerate(output) if 'Eth' in s]]
                for line in rates:
                    if line[2].find('B') == -1:
                        speed_exp = speed_exp_bits
                        bits=8
                    else:
                        speed_exp = speed_exp_bytes
                        bits=1
                    eth = line[0]
                    egress  = float(line[1])*(1000**(speed_exp.index(line[2])))/bits
                    ingress = float(line[4])*(1000**(speed_exp.index(line[5])))/bits
                            ...
                            for j,val in enumerate(row):
                                if j == 0:
                                    ...
                                    if exp < 3:
                                        ...
                                        if (man > 4.8 and not TenGBE) or (man > 1.2222222222222222222222 and TenGBE):
                                            col_pair = 7
                                            if reverse: col_pair += 1
